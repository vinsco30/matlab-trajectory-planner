function [qd, d_qd, dd_qd] = compute_cubic(ti, tf, qi, qf)
    ...
    % qi - Starting configuration [pos, vel] (R^2 column vector)
    % qf - Final configurations [pos, vel] (R^2 column vector)
    %
    % Outputs:
    ...
    for i = 1:length(t)
        qd(i) = coeff(1) + coeff(2)*t(i) + coeff(3)*t(i)^2 + coeff(4)*t(i)^3;
        d_qd(i) = coeff(2) + 2*coeff(3)*t(i) + 3*coeff(4)*t(i)^2;
        dd_qd(i) = 2*coeff(3) + 6*coeff(4)*t(i);
    end
